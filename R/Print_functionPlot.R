...
RDistributionPlot <- function(x, n, mean = 0, sd = 1, df = n - 2, dfn = 2, dfd = n - 2, size = 10, p = .1, seed = FALSE, bins = 10) {
  if (n %% 1 == 0) {
    if ((x == "Normal") | (x == "normal")) {
      out <- rnorm(n, mean, sd)
      hist(out, breaks = bins, xlab = "Normal Distribution Output", main = "Histogram of Normal Distribution", col = "red", border = "black")
    } else if ((x == "T") | (x == "t")) {
      out <- rt(n, df)
      hist(out, breaks = bins, xlab = "T Distribution Output", main = "Histogram of T Distribution", col = "green", border = "black")
    } else if ((x == "F") | (x == "f")) {
      out <- rf(n, dfn, dfd)
      hist(out, breaks = bins, xlab = "F Distribution Output", main = "Histogram of F Distribution", col = "yellow", border = "black")
    } else if ((x == "Binomial") | (x == "binomial") | (x == "binom")) {
      if (seed != FALSE) {
        if (seed %% 1 == 0) {
          set.seed(seed)
          out <- rbinom(n, size, p)
          hist(out, breaks = bins, xlab = "Binomial Distribution Output", main = "Histogram of Binomial Distribution", col = "blue", border = "black")
        }else {
